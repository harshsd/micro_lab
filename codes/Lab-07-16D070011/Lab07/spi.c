...
unsigned char serial_data,cursor_pos,display_num;
...
void main(void)
{
	...
	while(1)
	{
		...
		CS_BAR = 1;
		data_save_low = (data_save_low>>6); 
		tempadcVal = data_save_high;
		tempadcVal = (tempadcVal<<2); 
		tempadcVal += data_save_low;  
		//tempadcVal = (data_save_high<<2) + data_save_low;		// save the 10 bits in one variable
	}
}
/**
...
void timer0_ISR (void) interrupt 1
{
	...
	//Increment Overflow
	 ...
	 {
	 	avgVal = sum/10;
	 	adcVal = (avgVal*3.9*5 - 175)*1.045;
	 	cursor_pos = 0x83;
	 	for (i=0;i<=3;i++)
	 	{
	 		LCD_CmdWrite(cursor_pos);
	 		delay_ms(5);
	 		LCD_DataWrite((adcVal%10)+48);
	 		delay_ms(5);
	 		adcVal = adcVal/10;
